...
def modify_voter_data(vid, name, gender, zone, city, passw):
    df = pd.read_csv(path / 'voterList.csv')
    df.loc[df['voter_id'] == vid, 'Name'] = name
    df.loc[df['voter_id'] == vid, 'Gender'] = gender
    df.loc[df['voter_id'] == vid, 'Zone'] = zone
    df.loc[df['voter_id'] == vid, 'City'] = city
    df.loc[df['voter_id'] == vid, 'Passw'] = passw
